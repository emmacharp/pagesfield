<?php
	...
	require_once(EXTENSIONS . '/pagesfield/lib/class.entryquerypagesadapter.php');

	Class fieldPages extends Field{

		public function __construct(){
			parent::__construct();
			$this->entryQueryFieldAdapter = new EntryQueryPagesAdapter($this);
	...
	-------------------------------------------------------------------------*/
		...
		public static function fetchPageByTypes(array $types = array(), $andOperation = false, $negate = false) {
			...
			if(empty($types)) return (new PageManager)
				->select()
				->execute()
				->rows();
